...
class TestValidate(TestCase):
    ...
    def test_validate_language_extensions_invalid_foreach(self):
        ...
        # Verify the output contains an error message about the invalid Fn::ForEach syntax
        # The error should mention Fn::ForEach and indicate the syntax issue
        # Check for common error indicators related to Fn::ForEach validation failure
        error_indicators = [
            "Fn::ForEach",
            "ForEach",
            "Invalid",
        ...
        has_error_indicator = any(indicator.lower() in combined_output.lower() for indicator in error_indicators)
        self.assertTrue(
            has_error_indicator,
            f"Expected error message about invalid Fn::ForEach syntax. Actual output: {combined_output}",
        )
    ...
    def test_validate_language_extensions_cloud_dependent_collection(self):
        ...
        # Verify the output contains an error message about the cloud-dependent collection
        # The error should mention Fn::GetAtt or indicate the collection cannot be resolved
        error_indicators = [
            "GetAtt",
            "collection",
            "resolve",
        ...
        self.assertTrue(
            has_error_indicator,
            f"Expected error message about cloud-dependent collection. Actual output: {combined_output}",
        )
    ...
    def test_validate_language_extensions_missing_dynamic_artifact_dir(self):
        ...
        # sam validate performs basic SAM validation which may pass even if directories don't exist
        # The actual directory validation happens during sam build
        # This test documents the current behavior
        # If validation passes, that's acceptable for basic validation
        # If validation fails, it should have a clear error message
        if command_result.process.returncode != 0:
            # If it fails, verify there's a meaningful error message
            error_indicators = [
                "not found",
                "does not exist",
                "missing",
            ...
            self.assertTrue(
                has_error_indicator,
                f"Expected error message about missing directory. Actual output: {combined_output}",
            )
        # If it passes, that's also acceptable for basic validation
        # The directory check happens during sam build
