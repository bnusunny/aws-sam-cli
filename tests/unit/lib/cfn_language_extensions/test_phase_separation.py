...
from samcli.lib.samlib.wrapper import DynamicArtifactProperty, SamTranslatorWrapper
...
class TestExpandLanguageExtensions(TestCase):
    ...
    def test_original_template_is_deep_copy(self):
        """expand_language_extensions() original_template is a deep copy, not a reference."""
        template = {
            "Transform": "AWS::LanguageExtensions",
            "Resources": {
                "Fn::ForEach::Loop": [
        ...
        # original_template should not be the same object as the input
        self.assertIsNot(result.original_template, template)
        # But should have the same content
        self.assertEqual(result.original_template["Resources"], template["Resources"])
