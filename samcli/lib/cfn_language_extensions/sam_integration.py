"""
SAM Translator and SAM CLI integration for CloudFormation Language Extensions.

This module provides integration points for the AWS SAM ecosystem:
1. SAMLanguageExtensionsPlugin - Plugin for SAM Translator
...
def check_using_language_extension(template: Optional[Dict]) -> bool:
    ...
    if transform:
        if isinstance(transform, str) and transform.startswith(AWS_LANGUAGE_EXTENSIONS_TRANSFORM):
            return True
        if isinstance(transform, list):
            for transform_instance in transform:
                if not isinstance(transform_instance, str):
                    continue
                if transform_instance.startswith(AWS_LANGUAGE_EXTENSIONS_TRANSFORM):
                    return True
    return False
...
def detect_foreach_dynamic_properties(
    ...
    """
    from samcli.lib.samlib.wrapper import PACKAGEABLE_RESOURCE_ARTIFACT_PROPERTIES, DynamicArtifactProperty
...
def expand_language_extensions(
    ...
    # Deep copy original template before any modifications
...
class SAMLanguageExtensionsPlugin:
    """
    Plugin for SAM Translator to process language extensions.

    This plugin implements the SAM Translator plugin interface and processes
    CloudFormation language extensions before the SAM transform is applied.

    The plugin:
    1. Checks if AWS::LanguageExtensions is in the template's transforms
    2. Processes language extensions in partial resolution mode
    3. Removes AWS::LanguageExtensions from the transform list
    4. Returns the processed template for SAM Translator to continue

    This allows SAM templates to use language extensions like Fn::ForEach
    to generate multiple serverless resources.

    Example:
        >>> from samtranslator.plugins import LifeCycleEvents
        >>>
        >>> plugin = SAMLanguageExtensionsPlugin(parameter_values={"Env": "prod"})
        >>>
        >>> # Register with SAM Translator
        >>> translator.plugins.register(plugin)

    Requirements:
        - 14.1: Implement on_before_transform_template method
        - 14.2: Check for AWS::LanguageExtensions in transforms
        - 14.3: Process language extensions in partial mode
        - 14.4: Remove AWS::LanguageExtensions from transform list
        - 14.5: Preserve unresolvable refs for SAM processing
        - 14.6: Compatible with samtranslator.plugins interface
    """
    ...
    def __init__(
        ...
        """
        Initialize the SAM Language Extensions plugin.
    ...
    def on_before_transform_template(self, template: Dict[str, Any]) -> Dict[str, Any]:
        """
        SAM Translator lifecycle hook - called before SAM transform.

        This method is compatible with the samtranslator.plugins interface.
        It processes language extensions and removes the AWS::LanguageExtensions
        transform from the template before SAM Translator processes it.

        Args:
            template: The CloudFormation/SAM template dictionary.

        Returns:
            The processed template with language extensions resolved and
            AWS::LanguageExtensions removed from transforms.
        ...
        Requirements:
            ...
            - 14.4: Remove AWS::LanguageExtensions from transform list
        """
        # Get the transforms from the template
        ...
        # Check if AWS::LanguageExtensions is in transforms
        if AWS_LANGUAGE_EXTENSIONS_TRANSFORM not in transforms:
            # No language extensions to process
            return template

        # Process language extensions in partial mode
        context = TemplateProcessingContext(
            fragment=template.copy(),
            parameter_values=self.parameter_values,
            pseudo_parameters=self.pseudo_parameters,
            resolution_mode=ResolutionMode.PARTIAL,
        )

        pipeline = create_default_pipeline(context)
        processed = pipeline.process_template(context)

        # Remove AWS::LanguageExtensions from transforms
        new_transforms = [t for t in transforms if t != AWS_LANGUAGE_EXTENSIONS_TRANSFORM]

        # Update the Transform field based on remaining transforms
        if len(new_transforms) == 0:
            # No transforms left, remove the field
            processed.pop("Transform", None)
        elif len(new_transforms) == 1:
            # Single transform, use string form
            processed["Transform"] = new_transforms[0]
        else:
            # Multiple transforms, use list form
            processed["Transform"] = new_transforms
...
def process_nested_stacks(
    template: Dict[str, Any],
    parameter_values: Optional[Dict[str, Any]] = None,
    pseudo_parameters: Optional[PseudoParameterValues] = None,
    nested_templates: Optional[Dict[str, Dict[str, Any]]] = None,
) -> Dict[str, Dict[str, Any]]:
    """
    Process a template and its nested stacks.

    This function processes the main template and any nested stack templates
    independently. Each nested stack is processed with its own parameter
    values while sharing the same pseudo-parameters.

    Args:
        template: The main template dictionary.
        parameter_values: Parameter values for the main template.
        pseudo_parameters: AWS pseudo-parameter values (shared across stacks).
        nested_templates: Dictionary mapping nested stack logical IDs to
                          their template dictionaries.

    Returns:
        Dictionary mapping logical IDs to processed templates.
        The main template is keyed as "MainTemplate".

    Example:
        >>> main_template = load_template("main.yaml")
        >>> nested = {
        ...     "VPCStack": load_template("vpc.yaml"),
        ...     "DatabaseStack": load_template("database.yaml"),
        ... }
        >>> results = process_nested_stacks(
        ...     main_template,
        ...     parameter_values={"Env": "prod"},
        ...     nested_templates=nested
        ... )
        >>> processed_main = results["MainTemplate"]
        >>> processed_vpc = results["VPCStack"]

    Requirements:
        - 15.6: Process each nested stack independently
    """
    results: Dict[str, Dict[str, Any]] = {}

    # Process the main template
    results["MainTemplate"] = process_template_for_sam_cli(
        template,
        parameter_values=parameter_values,
        pseudo_parameters=pseudo_parameters,
    )

    # Process nested templates if provided
    if nested_templates:
        for logical_id, nested_template in nested_templates.items():
            # Each nested stack gets its own processing
            # Note: In a real implementation, we might extract parameter
            # values from the parent stack's AWS::CloudFormation::Stack
            # resource properties
            results[logical_id] = process_template_for_sam_cli(
                nested_template,
                parameter_values={},  # Nested stacks have their own params
                pseudo_parameters=pseudo_parameters,
            )
